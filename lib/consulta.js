 ...
 */
export function enriquecerArtigo(artigo, palavrasChave, enquadramentos) {
  ...
  if (enquadramentos && artigo.enquadramentos === undefined) {
    artigo.enquadramentos = enquadramentos;
  }
  return artigo;
}
 ...
 */
export function criarConsulta(lei, palavrasChave = {}, extras = {}) {
  const porNumero = new Map();
  for (const artigo of lei.artigos) {
    enriquecerArtigo(artigo, palavrasChave[artigo.numero],
      (extras.enquadramentos || {})[artigo.numero]);
  ...
  const indice = lei.artigos.map((artigo) => ({
    ...
    enquadNorm: normalizar((artigo.enquadramentos || []).map((e) => e.descricao).join(' | ')),
  }));
  ...
  function buscar(consulta) {
    ...
    const resultados = [];
    for (const { artigo: art, textoNorm, rubricasNorm, chavesNorm, enquadNorm } of indice) {
      const emChave = chavesNorm.includes(termo);
      const emRubrica = rubricasNorm.includes(termo);
      const emEnquadramento = enquadNorm.includes(termo);
      const posicao = textoNorm.indexOf(termo);
      if (!emChave && !emRubrica && !emEnquadramento && posicao === -1) continue;
      ...
      if (emEnquadramento && !trechos.length) {
        const enq = (art.enquadramentos || []).find((e) => normalizar(e.descricao).includes(termo));
        if (enq) trechos.push({ trecho: `Enquadramento ${enq.codigo}: ${enq.descricao}`, posicao: -1 });
      }
  ...
  function sugerir(consulta, limite = 8) {
    ...
    // palavra-chave | rubrica começa pelo termo | palavra da rubrica/enquadramento | palavra do texto | substring
    const grupos = [[], [], [], [], []];
    for (const { artigo: art, textoNorm, rubricasNorm, chavesNorm, enquadNorm } of indice) {
      const daRubrica = () => ({
        ...resumo(art),
      ...
      if (reInicioPalavra.test(enquadNorm)) {
        const enq = (art.enquadramentos || []).find((e) => reInicioPalavra.test(normalizar(e.descricao)));
        grupos[2].push({ ...resumo(art), descricao: `📋 ${enq ? enq.descricao : ''}` });
        continue;
      }
