...
export function criarConsulta(lei) {
  ...
  function sugerir(consulta, limite = 8) {
    ...
    // rubrica começa pelo termo | palavra da rubrica | palavra do texto | substring
    const grupos = [[], [], [], []];
    for (const { artigo: art, textoNorm, rubricasNorm } of indice) {
      const daRubrica = () => ({
        ...resumo(art),
      ...
      if (rubricasNorm.includes(termo)) grupos[3].push(daRubrica());
    }
    return [...grupos[0], ...grupos[1], ...grupos[2], ...grupos[3]].slice(0, limite);
  }
